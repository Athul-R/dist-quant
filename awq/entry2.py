...
# NEW: base (unquantized) evaluation path
parser.add_argument(
    "--base_eval",
    action="store_true",
...
assert (
    args.act_scale_path is not None and args.media_path and len(args.media_path) > 0
...
def build_model_and_enc(model_path, dtype):
    ...
    # Prepare tokenizer / config first
    if vila_10_quant_mode:
        from llava.model.builder import load_pretrained_model
        from llava.mm_utils import get_model_name_from_path

        # Tokenizer (enc) will come from load_pretrained_model
        enc = None
        config = None
    else:
    ...
    # --- Base (unquantized) evaluation path: skip any quantization/AWQ entirely ---
    if args.base_eval:
        ...
        if vila_10_quant_mode:
            ...
            enc2, model, image_processor, context_len = load_pretrained_model(
    ...
    if args.load_quant:  # directly load quantized weights
        ...
        model.eval()
    else:  # fp16/bf16 to quantized (or stay fp16/bf16 if no w_bit)
        ...
        if not vila_10_quant_mode:
            ...
            # if vila path, prefer tokenizer from llava loader
            enc = enc2
...
def main():
    ...
    # If base_eval is requested, forcibly disable AWQ/quantization knobs for safety.
    if args.base_eval:
        args.run_awq = False
        args.w_bit = None
        args.load_awq = None
    ...
    if args.tasks is not None:
        # Special-case Wikitext perplexity
        ...
        if args.output_path is not None:
            ...
            # ensure results exists in both branches
            if "results" not in locals():
                results = {"note": "no results object created"}
